...
def main():
    ...
    for index, example in enumerate(eval_data):
        ...
        print("Start insert!")
        # CodeBERT里迁移过来的att_attack需要5个参数：true_label, id2token, code_lines, adv_codes, query
        # LLM原本的taa_attack只需要3个参数：true_label, adv_codes, query
        # 为了适配迁移过来的代码，我们需要传入id2token和code_lines
        id2token = None # LLM不需要这个
        code_lines = None # LLM的get_importance_score_taa也不需要这个
        is_success, adv_code, query_times2 = attacker.att_attack(label, id2token, code_lines, new_code, query_times1)
        query_times = query_times2
        if not is_success == 1:
        ...
        success_attack += 1
        adv_label = 1 if label == 0 else 0
        recoder.write(index, code, adv_code, label, adv_label, query_times, round(example_end_time, 2), "AttInsert")
