...
from data_augmentation import get_identifiers
...
from ptm_attacker import Attention_Attacker
...
def main():
    ...
    code_tokens = []
    code_lines = []
    for _, code in enumerate(source_codes):
    ...
    attacker = Attention_Attacker(args, ptm_model, model, tokenizer)
    ...
    for index, example in enumerate(eval_dataset):
        ...
        success_attack += 1
        adv_label = 1 if label == 0 else 0
        recoder.write(index, code, adv_code, label, adv_label, query_times2, round(example_end_time, 2), "AttInsert")
        print("Success rate is : {}/{} = {}".format(success_attack, total_cnt, 1.0 * success_attack / total_cnt))
