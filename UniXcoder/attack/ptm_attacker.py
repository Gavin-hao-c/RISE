...
from python_parser.run_parser_CODA import get_identifiers_coda
...
def is_valid_substitue(substitute: str, tgt_word: str, lang: str) -> bool:
    return is_valid_variable_name(substitute, lang)
...
def get_substitues(substitutes, tokenizer, mlm_model, use_bpe, substitutes_score=None, threshold=3.0):
    ...
    if sub_len == 0:
        return words
    if sub_len == 1:
        if substitutes_score is None:
            substitutes_score = torch.ones_like(substitutes, dtype=torch.float)
        for (i, j) in zip(substitutes[0], substitutes_score[0]):
            if threshold != 0 and float(j) < threshold:
                break
