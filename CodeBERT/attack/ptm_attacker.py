...
from data_augmentation import get_identifiers, get_example, get_code_tokens, replace_function_name, get_function_call, query_code_type
import csv
import subprocess
import copy
import re
...
from python_parser.run_parser import  get_example, get_example_pos, get_identifiers_list, \
    is_valid_variable_c, get_all_snippet, remove_comments_and_docstrings, get_var, add_dec, get_snippet_token,get_example_batch,get_code_style, change_code_style,get_example_coda
...
#insert_dict= read_snippets('../../features/insert.jsonl')
#insert_dict= read_snippets('../../features/test_insert4_DiverVul.jsonl')
#insert_dict= read_snippets('../../features/test_insert4_BigVul.jsonl')
#insert_dict= read_snippets('../../features/insert_newBigVul.jsonl')
insert_dict= read_snippets('../../features/test_insert4_DiverVul2.jsonl') #good
#insert_dict= read_snippets('../../features/shorttest_insert_DiverseVul.jsonl')
#insert_dict= read_snippets('../../features/longtestlong_insert_DiverseVul.jsonl')

def get_importance_score_4line(args, ptm_model, code, label, tokenizer, type):
...
class SLODA_Attacker():
    ...
    def code_trans_attack(self, code, true_label, orig_probs, orig_pred):
        ...
        insert_positions = get_importance_score_4line(self.args, self.model_tgt, code, true_label,
    ...
    def taa_attack(self, true_label, id2token, code_lines, adv_codes, query):
        ...
        for adv_code in adv_codes:
            ...
            sorted_line_numbers = get_importance_score_4line(self.args, self.model_tgt, adv_code, true_label,
...
class Attention_Attacker():
    ...
    def _collect_host_identifiers(self, code):
        try:
            raw = get_identifiers(code)
        except Exception:
            return []
        uniq = []
        seen = set()
        for name in raw:
            ...
            if not s:
                continue
            if not is_valid_variable_name(s, 'c'):
                continue
            if s not in seen:
                seen.add(s)
                uniq.append(s)
    ...
    def _make_stealth_name(self, host_identifiers, used_names, fallback_base):
        pool = [x for x in host_identifiers if x not in used_names]
        if len(pool) >= 2:
            parts = random.sample(pool, 2)
        elif len(pool) == 1:
            parts = [pool[0]]
        else:
            parts = [fallback_base]
        candidate = "_".join(parts + [fallback_base])
        candidate = candidate[:80]
        if not is_valid_variable_name(candidate, 'c'):
            candidate = f"{fallback_base}_{random.randint(100, 999)}"
        while candidate in used_names or candidate in host_identifiers:
            candidate = f"{candidate}_{random.randint(100, 999)}"
            candidate = candidate[:80]
        return candidate

    def _stealthify_insert_code(self, host_code, insert_code):
        vars_in_snippet = insert_dict.get(insert_code, [])
        if not vars_in_snippet:
            return insert_code
        host_identifiers = self._collect_host_identifiers(host_code)
        if not host_identifiers:
            return insert_code
        mapping = {}
        used = set(host_identifiers)
        for v in vars_in_snippet:
            if not isinstance(v, str):
                continue
            old = v.strip()
            if not old:
                continue
            if not is_valid_variable_name(old, 'c'):
                continue
            new = self._make_stealth_name(host_identifiers, used, old)
            mapping[old] = new
            used.add(new)
        if not mapping:
            return insert_code
        replaced = insert_code
        for old, new in mapping.items():
            replaced = get_example(replaced, old, new)
        return replaced
    ...
    def att_attack(self, true_label, id2token, code_lines, adv_codes, query):
        query_times = query
        is_success = False
        if not adv_codes:
            return is_success, None, query_times
        insert_code_list = list(insert_dict.keys())
        if len(insert_code_list) == 0:
            return is_success, adv_codes[0], query_times
        final_code = adv_codes[0]
        for adv_code in adv_codes:
            sorted_line_numbers = get_importance_score_4line(
                self.args, self.model_tgt, adv_code, true_label, self.tokenizer_tgt, "insert"
            )
            begin_line = 0
            lines = adv_code.split('\n')
            for i, line in enumerate(lines):
                if '{' in line.strip():
                    ...
                    break
            valid_positions = list(range(begin_line, len(lines) - 1))
            current_code = adv_code
            iter_count = 0
            for position in sorted_line_numbers:
                if iter_count >= 7:
                    break
                if position not in valid_positions:
                    continue
                ...
                base_js = {"func": current_code, "target": true_label}
                base_feature = convert_examples_to_features(base_js, self.tokenizer_tgt, self.args)
                if self.ptm_model == "GraphCodeBERT":
                    base_dataset = GraphCodeDataset([base_feature])
                elif self.ptm_model == "EPVD":
                    base_dataset = EPVDDataset([base_feature])
                else:
                    base_dataset = CodeDataset([base_feature])
                base_logits, _ = self.model_tgt.get_results(base_dataset, self.args.eval_batch_size)
                query_times += 1
                if query_times > 1500:
                    return is_success, final_code, query_times
                base_prob = base_logits[0][true_label]

                best_code = None
                best_drop = 0
                for insert_code in insert_code_list:
                    stealth_insert = self._stealthify_insert_code(current_code, insert_code)
                    new_code = insert_at_line(current_code, position, stealth_insert)
                    temp_code_js = {"func": new_code, "target": true_label}
                    new_feature = convert_examples_to_features(temp_code_js, self.tokenizer_tgt, self.args)
                    if self.ptm_model == "GraphCodeBERT":
                        new_dataset = GraphCodeDataset([new_feature])
                    ...
                    if preds[0] != true_label:
                        return True, new_code, query_times
                    temp_prob = logits[0][true_label]
...
class MHM_Attacker():
    ...
    def __replaceUID_random(self, tokenizer, _tokens, _label=None, _uid={}, substitute_dict={},
        ...
        selected_uid = random.sample(list(substitute_dict.keys()), 1)[0]  # 选择需要被替换的变量名,随即替换
        if _candi_mode == "random":
            ...
            #将生成的代码候选通过模型转换成包括id和label等输入特征，接着转换成对应模型所处理的对象new_dataset， 最后调用模型得到预测结果及其概率
            new_example = []
...
load_json_path = "/root/autodl-tmp/SLODA/dataset/Devign"

class HG_Attack:
    def __init__(self, args, model_tgt, tokenizer_tgt, _ptm_model, _token2idx, _idx2token) -> None:
        self.args = args
        self.model_tgt = model_tgt
        self.tokenizer_tgt = tokenizer_tgt
        self.ptm_model = _ptm_model
    ...
    def random_replace(self, tokenizer, _tokens, _label=None, identifier=None, _n_candi=5, _max_iter=10):
        identifiers = identifier if identifier is not None else get_identifiers(_tokens)
        code_tokens = get_code_tokens(_tokens)
        processed_code = " ".join(code_tokens)
        words, sub_words, keys = _tokenize(processed_code, tokenizer)
        raw_tokens = copy.deepcopy(words)
        uid = get_identifier_posistions_from_code(words, identifiers)
        if len(uid) <= 0:
            return {'succ': None, 'tokens': None, 'raw_tokens': None, 'query_times': 0}
        variable_substitue_dict = {}
        for tgt_word in uid.keys():
            variable_substitue_dict[tgt_word] = random.sample(self.idx2token, _n_candi)
        old_uids = {}
        ...
        query_times = 0
        for iteration in range(1, 1 + _max_iter):
            res = self.__replaceUID_random(tokenizer, _tokens=_tokens, _label=_label, _uid=uid,
                                           substitute_dict=variable_substitue_dict,
                                           _n_candi=_n_candi,
            ...
            if res['status'].lower() in ['s', 'a']:
                ...
                if res['status'].lower() == 's':
                    ...
                    for uid_ in old_uids.keys():
                        replace_info[uid_] = old_uids[uid_][-1]
                        nb_changed_pos += len(uid[old_uids[uid_][-1]])
                    return {'succ': True, 'tokens': _tokens,
                            'raw_tokens': raw_tokens, "prog_length": len(code_tokens), "new_pred": res["new_pred"],
                            "is_success": 1, "old_uid": old_uid,
                            "nb_changed_var": len(old_uids), "nb_changed_pos": nb_changed_pos,
                            "replace_info": replace_info, "attack_type": "HGA", "query_times": query_times}
        replace_info = {}
        nb_changed_pos = 0
        for uid_ in old_uids.keys():
            replace_info[uid_] = old_uids[uid_][-1]
            nb_changed_pos += len(uid[old_uids[uid_][-1]])
        return {'succ': False, 'tokens': res['tokens'], 'raw_tokens': None, "prog_length": len(code_tokens),
                "new_pred": res["new_pred"], "is_success": -1, "old_uid": old_uid,
                "nb_changed_var": len(old_uids), "nb_changed_pos": nb_changed_pos, "replace_info": replace_info, "attack_type": "HGA", "query_times": query_times}
    ...
    def __replaceUID_random(self, tokenizer, _tokens, _label=None, _uid={}, substitute_dict={},
        ...
        if _candi_mode == "random":
            ...
            if self.ptm_model == "GraphCodeBERT":
                ...
                new_dataset = CodeDataset(new_example)
            prob, pred = self.model_tgt.get_results(new_dataset, self.args.eval_batch_size)
            ...
            return {"status": "r", "alpha": 0, "tokens": _tokens,
                    "old_uid": selected_uid, "new_uid": selected_uid,
                    "old_prob": None, "new_prob": None,
                    "old_pred": None, "new_pred": None, "nb_changed_pos": 0}

    def insertCheck_attack(self, tokenizer, position=0, code=None, _label=None, _n_lines=5):
        if code is None or tokenizer is None or _label is None:
            return "reject", code, code, 0
        temp_code_js = {"func": code, "target": _label}
        feature = convert_examples_to_features(temp_code_js, tokenizer, self.args)
        if self.ptm_model == "GraphCodeBERT":
            dataset = GraphCodeDataset([feature])
        elif self.ptm_model == "EPVD":
            dataset = EPVDDataset([feature])
        else:
            dataset = CodeDataset([feature])
        logits, preds = self.model_tgt.get_results(dataset, self.args.eval_batch_size)
        ori_prob = logits[0][_label]
        ori_pred = preds[0]
        lines = code.split("\n")
        start = max(0, position)
        end = min(len(lines), start + _n_lines)
        snippet = "\n".join(lines[start:end])
        candi_ident = list(set(get_identifiers(snippet)))
        query_times = 0
        best_drop = -1e9
        best_code = None
        for var in candi_ident:
            insert_stmt = f"if (!{var}) {{ int _err_flag = 1; }}"
            new_code = insert_at_line(code, start, insert_stmt)
            temp_js = {"func": new_code, "target": _label}
            new_feature = convert_examples_to_features(temp_js, tokenizer, self.args)
            if self.ptm_model == "GraphCodeBERT":
                new_dataset = GraphCodeDataset([new_feature])
            elif self.ptm_model == "EPVD":
                new_dataset = EPVDDataset([new_feature])
            ...
            query_times += 1
            new_prob = new_logits[0][_label]
            new_pred = new_preds[0]
            drop = ori_prob - new_prob
            if drop > best_drop:
                best_drop = drop
                best_code = new_code
        if best_code is not None and best_drop > 0:
            return "accept", code, best_code, query_times
        return "reject", code, code, query_times

    def funcWrap_attack(self, func_name, tokenizer,code=None, _label=None):
        try:
            with open(os.path.join(load_json_path, "function_type.json"), "r") as f:
                type_info = json.load(f)
        except Exception:
            return "reject", code
        if func_name not in type_info:
            return "reject", code
        orig_js = {"func": code, "target": _label}
        orig_feature = convert_examples_to_features(orig_js, tokenizer, self.args)
        if self.ptm_model == "GraphCodeBERT":
            orig_dataset = GraphCodeDataset([orig_feature])
        elif self.ptm_model == "EPVD":
            orig_dataset = EPVDDataset([orig_feature])
        else:
            orig_dataset = CodeDataset([orig_feature])
        orig_logits, orig_preds = self.model_tgt.get_results(orig_dataset, self.args.eval_batch_size)
        info = type_info[func_name]
        return_type = info.get("return_type", "void")
        param_types = info.get("param_types", [])
        safe_name = func_name + "_safe"
        code_replaced = replace_function_name(code, func_name, safe_name)
        ...
        if param_signature:
            decl = f"{return_type} {func_name}({param_signature})"
        else:
            decl = f"{return_type} {func_name}()"
        check_code = ""
        if param_names:
            first_param = param_names[0]
            rt = return_type.strip()
            if rt.startswith("int"):
                check_code = f"if (!{first_param}) return -1;\n    "
            else:
                check_code = f"if (!{first_param}) {{\n        int _err_flag = 1;\n    }}\n    "
        if call_args:
            if return_type == "void":
                body = "{\n    " + check_code + f"{safe_name}({call_args});" + "\n}"
            else:
                body = "{\n    " + check_code + f"return {safe_name}({call_args});" + "\n}"
        else:
            if return_type == "void":
                body = "{\n    " + check_code + f"{safe_name}();" + "\n}"
            else:
                body = "{\n    " + check_code + f"return {safe_name}();" + "\n}"
        wrapper_code = decl + "\n" + body + "\n"
        new_code = wrapper_code + code_replaced
        temp_code_js = {"func": new_code, "target": _label}
        feature = convert_examples_to_features(temp_code_js, tokenizer, self.args)
        if self.ptm_model == "GraphCodeBERT":
            dataset = GraphCodeDataset([feature])
        elif self.ptm_model == "EPVD":
        ...
        orig_prob = orig_logits[0][_label]
        new_prob = logits[0][_label]
        if new_prob < orig_prob:
            return "accept", new_code
        return "reject", new_code
    ...
    def Hybrid_attack(self, code, tokenizer, _label=None):
        if code is None or tokenizer is None or _label is None:
            return "failed", code, 0, None
        total_queries = 0
        identifiers = get_identifiers(code)
        code_tokens = get_code_tokens(code)
        processed_code = " ".join(code_tokens)
        ...
        if len(variable_names) > 0:
            ...
            if importance_score is not None:
                ...
                top_identifiers = [item[0] for item in sorted_list_of_names[:5]]
        current_code = code
        if len(top_identifiers) > 0:
            res_random = self.random_replace(tokenizer, current_code, _label=_label, identifier=top_identifiers)
            total_queries += res_random.get("query_times", 0)
            if res_random.get("succ"):
                adv_code = res_random.get("tokens", current_code)
                return "success", adv_code, total_queries, "replace"
            if res_random.get("succ") is False and res_random.get("tokens") is not None:
                current_code = res_random.get("tokens")
        line_indices = get_importance_score_4line(self.args, self.model_tgt, current_code, _label, tokenizer, "replace")
        if not line_indices:
            return "failed", current_code, total_queries, None
        selected_lines = sorted(line_indices[:10])
        line_offset = 0
        for original_line in selected_lines:
            target_line = original_line + line_offset
            lines = current_code.split("\n")
            if target_line < 0 or target_line >= len(lines):
                continue
            line_text = lines[target_line]
            code_type = query_code_type(current_code, line_text)
            if code_type == "Function Calls":
                func_name = self._extract_func_name(line_text)
                if not func_name:
                    continue
                status, new_code = self.funcWrap_attack(func_name, tokenizer, code=current_code, _label=_label)
                total_queries += 1
                if status == "success":
                    return "success", new_code, total_queries, "funcWrap"
                if status == "accept":
                    delta = new_code.count("\n") - current_code.count("\n")
                    current_code = new_code
                    line_offset += delta
            elif code_type in {"Control Flow Statements", "Assignments"}:
                status, _, new_code, q = self.insertCheck_attack(tokenizer, position=target_line, code=current_code, _label=_label)
                total_queries += q
                if status == "accept":
                    temp_code_js = {"func": new_code, "target": _label}
                    feature = convert_examples_to_features(temp_code_js, tokenizer, self.args)
                    ...
                    total_queries += 1
                    if preds[0] != _label:
                        return "success", new_code, total_queries, "insertCheck"
                    delta = new_code.count("\n") - current_code.count("\n")
                    current_code = new_code
                    line_offset += delta
        return "failed", current_code, total_queries, None

def transform_code4_RTI(code, label, insert_position):
    new_codes = []
    for i in range(4):
        new_code=mutation_tra(code, i, insert_position)
        if new_code != "":
            new_codes.append(new_code)
    new_codes = set(new_codes)
    new_codes = list(new_codes)
...
# def is_line_incomplete(line: str) -> bool:
#     """判断一行代码是否明显不完整"""
#     stripped = line.strip()
#     if not stripped or stripped.endswith(';'):
#         return False  # 空行或以分号结尾，视为完整（C 语句通常以 ; 结尾）
...
#     # 检查是否以常见双字符运算符结尾（需特别处理）
#     if any(stripped.endswith(op) for op in ["==", "!=", "<=", ">=", "&&", "||", "<<", ">>"]):
#         return True

#     # 如果没有分号，且不是控制结构开头（如 if/for/while），也可能是不完整
#     # 但为了保守起见，我们主要依赖上述符号规则
    
#     return False

# def insert_code_generation(adv_context):
#     tokenizer_Gen = AutoTokenizer.from_pretrained("Salesforce/codegen-350M-multi")
#     model_Gen = AutoModelForCausalLM.from_pretrained("Salesforce/codegen-350M-multi")
#     inputs_Gen  = tokenizer_Gen(adv_context, return_tensors="pt")    
#     generated_code = model_Gen.generate(input_ids = inputs_Gen.input_ids,attention_mask = inputs_Gen.attention_mask,max_length=128,
#                                                     pad_token_id = tokenizer_Gen.eos_token_id,num_return_sequences=1,do_sample=False)
#     full_output = tokenizer_Gen.decode(generated_code[0], skip_special_tokens=True)
#     if full_output.startswith(adv_context):
#         generated_only = full_output[len(adv_context):]
#     else:
#         # 容错：尝试去除公共前缀（简化处理）
#         generated_only = full_output
#         # 更鲁棒可用 difflib，但此处假设基本对齐
...
#         # 规则2: 禁止 return / break / continue
#         words = stripped.split()
#         if any(kw in words for kw in ['return', 'break', 'continue']):
#             continue
...
def is_incomplete(line: str) -> bool:
    ...
    if not s or s.endswith(';'):
        return False
    bad_endings = ["'", '"', ",", "(", "[", "{", "+", "-", "*", "/", "=", "<", ">", "&", "|", "^", "!"]
    if any(s.endswith(e) for e in bad_endings):
        return True
    # 双字符运算符结尾也视为不完整
    multi_ops = ["==", "!=", "<=", ">=", "&&", "||", "<<", ">>"]
    if any(s.endswith(op) for op in multi_ops):
        return True
    return False
...
def insert_code_generation(adv_context):
    ...
    # 生成代码
    inputs = tokenizer(adv_context, return_tensors="pt")
    generated_ids = model.generate(input_ids=inputs.input_ids,attention_mask=inputs.attention_mask,
                                   max_new_tokens=60,pad_token_id=tokenizer.eos_token_id,do_sample=False)
    # 解码完整输出
    ...
    # 容错处理：尝试移除 prompt（适用于空格/换行微小差异）
        generated_only = full_output
    # 更强容错可用 difflib，但此处简化
    valid_lines = []
    for line in generated_only.split('\n'):
        ...
        if '}' in line:
            continue
        # 跳过含 return / break / continue 的行（作为独立词）
        words = stripped.split()
        if any(kw in words for kw in ['return', 'break', 'continue','printf','print']):
            continue
        # 跳过不完整行
        ...
        if len(valid_lines) >= MAX_OUTPUT_LINES:
            break
...
class RTI_Attacker():
    def __init__(self, args, model_tgt, tokenizer_tgt,_ptm_model, _token2idx, _idx2token) -> None:
        ...
        self.token2idx = _token2idx
        self.idx2token = _idx2token
        self.model_tgt = model_tgt  #加入，参考WIR_Attacker
        self.tokenizer_tgt = tokenizer_tgt #加入
        self.args = args

    def mcmc_random(self, example,tokenizer, code=None, _label=None, _n_candi=30,
                    _max_iter=100, _prob_threshold=0.95):
        identifiers = get_identifiers(code)
        code_tokens = get_code_tokens(code)
        processed_code = " ".join(code_tokens)
        ...
        sorted_list_of_names = sorted(names_to_importance_score.items(), key=lambda x: x[1], reverse=True)
        top_identifiers = [item[0] for item in sorted_list_of_names[:_n_candi]]
        uid = get_identifier_posistions_from_code(words, top_identifiers)
        # UID = {'a': [0, 8], 'b': [2, 10], 'c': [6, 13]}
        if len(uid) <= 0:  # 是有可能存在找不到变量名的情况的.
            return {'succ': None, 'tokens': None, 'raw_tokens': None}
        ...
        #想把sorted_list_of_names 变成类似uid 的样子，目前是从sorted_list_of_names中获得了identifier，在调用一遍
            ...
            if res['status'].lower() in ['s', 'a']:
                ...
                if res['status'].lower() == 's':
                    ...
                    return {'succ': True, 'tokens': code,
                            ...
                            "is_success": 1, "old_uid": old_uid, "score_info": res["old_prob"][0] - res["new_prob"][0],
                            "nb_changed_var": len(old_uids), "nb_changed_pos": nb_changed_pos,
                            "replace_info": replace_info, "attack_type": "replace identifiers by importance score", "query_times":query}
        replace_info = {}
        nb_changed_pos = 0
        ...
        return {'succ': False, 'tokens': res['tokens'], 'raw_tokens': None, "prog_length": prog_length,
                "new_pred": res["new_pred"], "is_success": -1, "old_uid": old_uid,
                "score_info": res["old_prob"][0] - res["new_prob"][0], "nb_changed_var": len(old_uids),
                "nb_changed_pos": nb_changed_pos, "replace_info": replace_info, "attack_type": "replace identifiers by importance score","query_times":query}
    ...
    def __replaceUID_random(self, tokenizer, _tokens, _label=None, _uid={}, substitute_dict={},
        ...
        if _candi_mode == "random":
            # First, generate candidate set.
            # The transition probabilities of all candidate are the same.
            candi_token = [selected_uid]        ## ← 这是【候选变量名】列表
            candi_tokens = [copy.deepcopy(_tokens)]
            candi_labels = [_label]
            for c in random.sample(self.idx2token, _n_candi):  # 选出_n_candi数量的候选.
                if c in _uid.keys():
    ...
    def code_trans_attack(self, code, true_label, orig_probs, orig_pred):
        ...
        for i, insert_position in enumerate(insert_positions):
            ...
            new_codes = transform_code4_RTI(code, true_label, insert_position)
            # print(insert_position)
            try:
                new_codes.remove(code)
            except:
    ...
    def similar_code_insert_attack(self, true_label, id2token, code_lines, adv_codes, query):
        query_times = query
        is_success = -1
        existing_insert_code = []
        identifiers = get_identifiers(adv_codes[0])
        ...
        best_insert_code = None
        best_logits = None
        get_adv_codes = []  #插入后生成的新代码
        insert_code_list = []   #插入的代码片段
        #得到插入的重要性分数
        MAX_OUTPUT_LINES = 2
        # 加载模型
        result = subprocess.run('bash -c "source /etc/network_turbo && env | grep proxy"', shell=True, capture_output=True, text=True)
        ...
        for line in output.splitlines():
            if '=' in line:
                var, value = line.split('=', 1)
                os.environ[var] = value 
        tokenizer4GEN = AutoTokenizer.from_pretrained("Salesforce/codegen-350M-multi")
        model4GEN = AutoModelForCausalLM.from_pretrained("Salesforce/codegen-350M-multi") 
        ...
        for adv_code in adv_codes:
            ...
            for i, position in enumerate(sorted_line_numbers):
                ...
                if position not in valid_positions:
                    continue
                #得到当前位置的上下文, position 是从0 还是从1 开始？ 从0开始，这个重要性是在每一行后插入<unk>
                adv_context=""
                for i, line in enumerate(lines):
                    if i == 0:
                        adv_context = line
                ...
                inputs = tokenizer4GEN(adv_context, return_tensors="pt")
                generated_ids = model4GEN.generate(input_ids=inputs.input_ids,attention_mask=inputs.attention_mask,
                                   max_new_tokens=60,pad_token_id=tokenizer4GEN.eos_token_id,do_sample=False)
                full_output = tokenizer4GEN.decode(generated_ids[0], skip_special_tokens=True)
                if full_output.startswith(adv_context):
                    generated_only = full_output[len(adv_context):]
                else:
                    generated_only = full_output
                ...
                generate_code = '\n'.join(valid_lines).rstrip()
                #生成代码
                #generate_code = insert_code_generation(adv_context) #生成待插入的代码片段
                get_adv_code = insert_at_line(adv_code,position, generate_code)
                if get_adv_code not in get_adv_codes:
                    get_adv_codes.append(get_adv_code)
                    insert_code_list.append(generate_code)
                temp_code_js = {"func": get_adv_code, "target": true_label}
                new_feature = convert_examples_to_features(temp_code_js, self.tokenizer_tgt, self.args)
                new_dataset = None
                if self.ptm_model == "GraphCodeBERT":
                    new_dataset = GraphCodeDataset([new_feature])
                ...
                if temp_label != true_label:
                    is_success = 1
                    print("%s SUC! (%.5f => %.5f)" % ('>>', true_label, temp_label), flush=True)
                    return is_success, get_adv_code, query_times
                if query_times > 1500:
                    return is_success, None, query_times
                if best_logits is None or max(logits[0]) < best_logits:
                    if generate_code not in existing_insert_code:
                        best_logits = max(logits[0])
                        best_code = get_adv_code
                        best_insert_code = generate_code
...
class CODA_Attacker:
    ...
    def attack(self, example, code,query):
        ...
        if self.fasttext_model is None:
            var_pool = list(temp_subs_variable_name)
            func_pool = list(temp_subs_function_name)
            for i in variable_names:
                cand = [x for x in var_pool if x != i]
                random.shuffle(cand)
                substituions[i] = cand
            for i in function_names:
                cand = [x for x in func_pool if x != i]
                random.shuffle(cand)
                substituions[i] = cand
        else:
